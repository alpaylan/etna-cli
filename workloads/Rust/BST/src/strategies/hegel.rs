...
pub fn draw_case(property: &str, tc: &TestCase) -> Option<(String, Option<bool>)> {
    let out = match property {
        ...
        "UnionValid" => {
            let t1 = draw_tree(tc);
            let t2 = draw_tree(tc);
            (format!("({} {})", t1, t2), spec::prop_union_valid(t1, t2))
        }
        "InsertPost" => {
            let t = draw_tree(tc);
            let k = draw_i32(tc);
        ...
        "UnionModel" => {
            let t1 = draw_tree(tc);
            let t2 = draw_tree(tc);
            (format!("({} {})", t1, t2), spec::prop_union_model(t1, t2))
        }
        "InsertInsert" => {
            let t = draw_tree(tc);
            let k = draw_i32(tc);
        ...
        "UnionUnionIdempotent" => {
            let t = draw_tree(tc);
            (format!("({})", t), spec::prop_union_union_idempotent(t))
        }
        "UnionUnionAssoc" => {
            let t1 = draw_tree(tc);
            let t2 = draw_tree(tc);
